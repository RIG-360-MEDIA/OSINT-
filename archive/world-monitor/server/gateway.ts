...
export function createDomainGateway(
  ...
  return async function handler(originalRequest: Request): Promise<Response> {
    ...
    // Execute handler with top-level error boundary + 20s timeout guard
    const REQUEST_TIMEOUT_MS = 20_000;
    let response: Response;
    try {
      const timeoutPromise = new Promise<Response>((_, reject) =>
        setTimeout(() => reject(new Error('Handler timeout')), REQUEST_TIMEOUT_MS),
      );
      response = await Promise.race([matchedHandler(request), timeoutPromise]);
    } catch (err) {
      const isTimeout = err instanceof Error && err.message === 'Handler timeout';
      if (isTimeout) {
        console.warn('[gateway] Handler timed out:', pathname);
        response = new Response(JSON.stringify({ error: 'Request timed out', upstreamUnavailable: true }), {
          status: 504,
          headers: { 'Content-Type': 'application/json' },
        });
      } else {
