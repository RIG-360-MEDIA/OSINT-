...
// panel count limits removed — all panels unlocked
...
// auth-state imports removed — no sign-in required
...
export class EventHandlerManager implements AppModule {
  ...
  private performUndo(): void {
    ...
    if (!config) return;
    // No panel limit — all features unlocked
    config.enabled = true;
  ...
  private initFooterDownload(): void {
    // Download app button removed — not needed for local instance
  ...
  setupExportPanel(): void {
    ...
    // All features unlocked — always show export
    el.style.display = '';
  }
  ...
  setupUnifiedSettings(): void {
    this.ctx.unifiedSettings = new UnifiedSettings({
      ...
      toggleSource: (name: string) => {
        ...
        // No source limit — all features unlocked
        if (reenabling) {
  ...
  setupPlaybackControl(): void {
    ...
    // All features unlocked — always show playback
    el.style.display = '';
  }
