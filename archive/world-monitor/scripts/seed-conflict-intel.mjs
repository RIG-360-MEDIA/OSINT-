...
async function fetchAcledEvents() {
  ...
  // Try progressively wider windows — ACLED data may lag weeks/months behind real time.
  // Lookbacks: 30d → 90d → 180d → 400d → 730d
  const LOOKBACKS_DAYS = [30, 90, 180, 400, 730];
  let rawEvents = [];
  let startDate = '';
  let endDate = new Date(now).toISOString().split('T')[0];

  for (const days of LOOKBACKS_DAYS) {
    ...
    console.log(`  ACLED: 0 events in last ${days}d window, widening...`);
  }
