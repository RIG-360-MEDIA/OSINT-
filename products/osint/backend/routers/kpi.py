...
async def get_kpi() -> dict[str, Any]:
    async with get_db() as db:
        row = (await db.execute(text("""
            WITH last24 AS (
              SELECT a.id, a.language_iso, a.source_id
                FROM articles a
            ...
            SELECT
              ...
              (SELECT COUNT(DISTINCT language_iso) FROM last24
                WHERE language_iso IS NOT NULL)                         AS languages,
        ...
        langs = (await db.execute(text("""
            SELECT UPPER(language_iso) AS code, COUNT(*) AS n
              FROM articles
