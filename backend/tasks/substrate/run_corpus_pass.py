...
def parse_html(html: str, article_url: str) -> dict[str, Any]:
    ...
    # ─── canonical url ────────────────────────────────────────────────
    # B4 patch (2026-05-26): reject homepage-collapse (e.g. Morung Express
    # serves <link rel=canonical href=https://www.morungexpress.com/> on
    # every article, which corrupted 1,222 rows before we caught it).
    canonical = None
    link = soup.find("link", rel="canonical")
    if link and link.get("href"):
        cand = (link["href"] or "").strip()
        # Require a meaningful path (not just "/" or empty)
        try:
            from urllib.parse import urlparse
            parsed = urlparse(cand)
            path = (parsed.path or "").rstrip("/")
            if path and len(path) > 1:  # at least "/a" — homepage collapses to ""
                canonical = cand
        except Exception:
            canonical = None
