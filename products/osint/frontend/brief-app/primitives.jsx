...
const MetricNumber = ({ value, format = "int", duration = 1100, className = "" }) => {
...
/* ── StanceDot ────────────────────────────────────────────── */
const StanceDot = ({ stance, size = 8, style = {} }) => {
  const colors = {
    supportive: "#34d399",
    critical: "#fb7185",
    neutral: "#a78bfa",
...
/* ── LiveDot ──────────────────────────────────────────────── */
const LiveDot = ({ tone = "live" }) => <span className={`live-dot ${tone}`}></span>;

/* ── Countdown (MO-5: real refresh countdown) ──────────── */
const Countdown = ({ to, bare = false }) => {
...
/* ── Section Header ───────────────────────────────────────── */
const SectionHead = ({ icon, label, subtitle, accent = "#e9c46a" }) => (
...
const ImageSlot = ({ src, alt, kind = "rect", id, label, className = "" }) => {
...
const MethodPopover = ({ title, children, placement = "top" }) => {
...
Object.assign(window.RIG, {
  Icon, Sparkline, MetricNumber, StanceDot, LanguagePill, LiveDot, Countdown, Reveal, SectionHead, ImageSlot, MethodPopover });
