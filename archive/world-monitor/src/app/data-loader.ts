...
export class DataLoaderManager implements AppModule {
  ...
  async loadAllData(forceAll = false): Promise<void> {
    ...
    if (SITE_VARIANT === 'full') {
      try {
        ...
        if (cached && cached.cii.length > 0) {
          // Seed country-instability module so theater posture recalc has CII
          // data even when live ACLED/UCDP event APIs are unavailable.
          setServerCiiScores(cached.cii);
    ...
    // Fire-and-forget: LLM calls — don't block main load pipeline
    if (hasPremiumAccess()) {
      this.loadDailyMarketBrief().catch(() => {});
      this.loadMarketImplications().catch(() => {});
    }
