"""brief_entities.py — live data for the 4 Watched Entity cards.

For each tracked entity we compute:
  - mentions_today      from entity_mention_daily (T6)
  - change_pct          vs 7-day baseline
  - sentiment           avg article_stances.intensity in last 24h
  - velocity / class    hot/rising/stable bucket
  - velocityBars        last 15 hours of mention counts (sparkline)
  - latest_quote + ctx  most-recent article_quotes row
"""
...
# Boss's mockup tracks these 4. Each carries:
#   - entity_uuid : FK to entity_dictionary.id (preferred — set by Option C bulk-link)
#   - patterns    : tightened ILIKE fallback (multi-word, avoids surname collisions)
...
def _classify_velocity(change_pct: float | None) -> tuple[str, str]:
    """Return (label, sentiment_label_when_unknown)."""
...
async def _one_entity(db, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    # Build hybrid match: FK first (zero false positives), ILIKE fallback only if FK NULL
    ...
    # Mentions today + baseline + sentiment in ONE query
    ...
    quote_row = (await db.execute(text(f"""
        SELECT aq.quote_text, aq.collected_at, s.name AS source
          FROM article_quotes aq
          JOIN articles a ON a.id = aq.article_id
          JOIN sources s ON s.id = a.source_id
         WHERE ({or_quotes})
         ORDER BY a.collected_at DESC LIMIT 1
    """), params)).fetchone() if False else None  # disable for now — quote_text has subjects, lots of cleanup needed; use article_claims instead

    # C+D+E quote-quality filter:
    #   E — drop short fragments (< 30 chars) and known noise patterns
    #   D — prefer longer, well-formed quotes over short fragments
    #   C — return only 1 best, picked by quality score (length capped at 240)
    # Recency still matters but only as tie-breaker after quality.
    quote_row = (await db.execute(text(f"""
        SELECT aq.quote_text, a.collected_at, s.name AS source
         ...
         WHERE ({or_quotes})
           ...
           AND aq.quote_text !~ '^[A-Z][a-z]+,\\s+[A-Z][a-z]+\\s*$'  -- skip "Adani, Ambani" style
    ...
    # Velocity sparkline — last 15 hourly mention counts
    ...
    # Influence: rough — clip mention count to 0-100
...
async def get_watched_entities(db) -> dict[str, Any]:
    items = []
    for cfg in ENTITIES_CONFIG:
        items.append(await _one_entity(db, cfg))
    return {
        "entities": items,
