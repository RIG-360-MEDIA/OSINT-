...
def render_html(r: dict[str, Any]) -> str:
    ...
    o.append(
        ...
        f"<div class='ks'>Web <b>{bp['web']}</b> &middot; Papers <b>{bp['newspaper']}</b> &middot; TV <b>{bp['tv']}</b></div></div>"
        f"<div class='kt'><div class='kl'>Biggest Subject</div><div class='kv'>{_e(s['biggest_subject']['topic'] or '—')}</div>"
        f"<div class='ks'>{s['biggest_subject']['items']} stories &middot; {s['about_government']} about the government</div></div>"
