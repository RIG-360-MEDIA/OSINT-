...
Ported from backend/observability/brief_stories.py. Same event_clusters →
article_events join, importance-ranked, with momentum bars + lens.
"""
...
async def get_stories(limit: int = 5) -> dict[str, Any]:
    async with get_db() as db:
        ...
        stories = [await _one_cluster(db, r, i) for i, r in enumerate(rows)]
