...
// Server-seeded CII scores used as fallback when no live event data is available
// (e.g. ACLED/UCDP APIs returning 403/401). Populated by setServerCiiScores().
const serverCiiOverrides = new Map<string, number>();

/** Call this after fetching cached risk scores from the API so theater posture
 *  recalculation has baseline CII data even when live event APIs are down. */
export function setServerCiiScores(scores: { code: string; score: number }[]): void {
  for (const s of scores) {
    if (typeof s.code === 'string' && Number.isFinite(s.score)) {
      serverCiiOverrides.set(s.code, s.score);
    }
  }
}
...
export function getCountryScore(code: string): number | null {
  const data = countryDataMap.get(code);
  // Fall back to server-seeded score when no live event data has been ingested
  if (!data) return serverCiiOverrides.get(code) ?? null;
