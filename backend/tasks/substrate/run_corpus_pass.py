...
async def process_one(db, article: dict[str, Any]) -> dict[str, Any]:
    ...
    if quality == "low" or word_count < 60 or title_says_junk(title):
        # short-circuit before we burn a Groq call.
        #
        # 2026-05-31 LABEL FIX: stop dumping real articles with a thin/empty
        # extracted body into 'junk'. Audit (docs/qa) found 87% of 'junk' corpus-wide
        # were real news whose body extraction yielded <60 words (favor_precision
        # + per-source fetch gaps); 95% are already embedded + clusterable. Split:
        #   title_says_junk           -> 'junk'         (genuine; is_junk_url handled above)
        #   quality 'low', real title -> 'low_quality'  (extraction failure, NOT junk —
        #                                a distinct, queryable target for a future
        #                                re-fetch/re-extraction pass)
        #   short but acceptable      -> 'ok'           (unchanged)
        if title_says_junk(title):
            short_status = "junk"
        elif quality == "low":
            short_status = "low_quality"
        else:
            short_status = "ok"
        ...
        await db.commit()
        return {"status": "ok_no_groq" if short_status == "ok" else short_status}
...
async def run(args: argparse.Namespace) -> int:
    ...
    fetched_q = text(
        """
        UPDATE articles
           SET substrate_status = 'processing', updated_at = now()
         WHERE id IN (
    ...
    started = time.time()
    counters = {"ok": 0, "junk": 0, "low_quality": 0, "fetch_failed": 0, "errors": 0, "ok_no_groq": 0}
    processed = 0
